...
//! Reorderable flex row widget implementation.
...
use cosmic::iced::core::{
    Background, Border, Clipboard, Length, Padding, Point, Rectangle, Shadow, Shell, Size, Vector,
    Widget,
};
use cosmic::iced::{Alignment, Pixels};
...
pub struct FlexRow<'a, Key, Message>
where
    Key: Clone + Eq + Hash + 'static,
{
    spacing: f32,
...
impl<'a, Key, Message> FlexRow<'a, Key, Message>
where
    Key: Clone + Eq + Hash + 'static,
{
    pub fn new(on_reorder: impl Fn(usize, usize) -> Message + 'a) -> Self {
        Self {
            spacing: 0.0,
    ...
    fn sync_slot_positions(&self, state: &mut State<Key>, slots: &[LocalSlot<Key>]) {
        ...
        let target_index = target_index_for_drag(
            &ordered_keys,
            &locked_by_key,
            &drag_key,
            dragged_center,
...
impl<'a, Key, Message> Widget<Message, cosmic::Theme, cosmic::Renderer>
    for FlexRow<'a, Key, Message>
where
    Key: Clone + Eq + Hash + 'static,
    Message: 'a,
{
            ...
            Event::Mouse(mouse::Event::ButtonReleased(mouse::Button::Left)) => {
                ...
                if let Some(drag) = state.drag.take() {
                    if drag.current_index != drag.original_index {
                        shell.publish((self.on_reorder)(drag.original_index, drag.current_index));
                    }
                    shell.capture_event();
                    shell.request_redraw();
                    return;
...
impl<'a, Key, Message> From<FlexRow<'a, Key, Message>> for Element<'a, Message>
where
    Key: Clone + Eq + Hash + 'static,
    Message: 'a,
{
    fn from(row: FlexRow<'a, Key, Message>) -> Self {
        Element::new(row)
    }
}

pub fn flex_row<'a, Key, Message>(
    on_reorder: impl Fn(usize, usize) -> Message + 'a,
) -> FlexRow<'a, Key, Message>
where
    Key: Clone + Eq + Hash + 'static,
{
    FlexRow::new(on_reorder)
}
...
where
    Key: Clone + Eq + Hash + 'static,
{
    let movable_count = ordered_keys
        .iter()
        .filter(|key| !locked_by_key.get(*key).copied().unwrap_or(false))
        .count();
    ...
    for target_index in 0..movable_count {
        let candidate_keys =
            reordered_keys_for_drag(ordered_keys, locked_by_key, dragged_key, target_index);
        let (candidate_slots, _) = compute_wrapped_slots(
            &candidate_keys,
            locked_by_key,
            size_by_key,
            wrap_width,
        ...
        let Some(slot) = candidate_slots.iter().find(|slot| &slot.key == dragged_key) else {
            continue;
        };
        ...
        let distance =
            (slot_center.x - dragged_center.x).powi(2) + (slot_center.y - dragged_center.y).powi(2);
...
fn draw_drag_backdrop(renderer: &mut cosmic::Renderer, theme: &cosmic::Theme, bounds: Rectangle) {
    let cosmic = theme.cosmic();
    let backdrop = cosmic::iced::Color {
        a: 0.08,
...
mod tests {
    ...
    use cosmic::iced::{Alignment, Padding, Point, Size};
